<template>
<div>
    <hjx-header label="D管理 / tree"></hjx-header>
    <el-form :inline="true">
        <el-form-item label="请选择D4：">
            <el-select v-model="strRelationId" filterable placeholder="请选择D4">
                <el-option  v-for="item in D4List"  :label="item.strUserName + ' / ' + item.strPhoneNum" :value="item.strRelationId" :key="item.strRelationId">
                </el-option>
            </el-select>
        </el-form-item>
	...
	export default {
        ...
        watch: {
            ...
            'relationIdFn' : function(val){ //获取可替换列表 / 下级列表
                if(!val) return
                this.getAddNextList(val)
                this.getReplaceDxList(val)
            }
        },
		mounted()  {
            this.index()
		...
		methods:{
            ...
            //刷新D4List
            getD4List(){
                api.getD4List({'strDType':'D4'}).then(res =>{
                    if (res.ret != '0') {
                        this.$message(res.retinfo)
                        return
                    }
                    this.D4List = res.dxlist
            ...
            delThisNode(strRelationId){
                ...
                api.delLeafNode({'strRelationId':strRelationId}).then(res =>{
                    ...
                    this.$message('成功')
                    if(this.replaceNodeForm.level == 'D4'){
                        this.treeList = []
                        this.strRelationId = ''
                        this.relationIdFn = ''
                        this.replaceNodeForm.strUserId = ''
            ...
            /*新增下级*/
            getAddNextList(strRelationId){
                api.getAddNextList({'strRelationId':strRelationId}).then(res =>{
                    if (res.ret != '0') {
                        this.$message(res.retinfo)
                        return
                    }
            ...
            submitForm:function(whichForm , judge){
                ...
                if(judge == 'AddNextForm'){
                    ...
                    api.replaceDxNode(data).then(res => {
                        ...
                        this.$message("成功！")
                        this.reloadList( this.replaceNodeForm.level == 'D4' )
                    })
                }
            },
            cancelnow: function() {
